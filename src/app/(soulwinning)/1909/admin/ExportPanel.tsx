...
import { fetchLeaderboard, fetchOverview } from "@/lib/soulwinning/admin";
import { buildSummaryPdf, loadLogo, type GroupSummary } from "@/lib/soulwinning/summary-pdf";
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="rounded-xl border border-[#e8e6e5] bg-white p-5 lg:col-span-2">
        ...
        <p className="mb-4 text-xs text-[#a8a29e]">
          The day on one sheet: the total against the goal, tongues and church, groups, top
          fellowships, PFCCs and members. No names of the souls.
        </p>
        <button
          type="button"
          disabled={busy !== null}
          onClick={async () => {
            ...
            try {
              ...
              const [overview, fellowships, pfccs, members, logo] = await Promise.all([
              ...
              const grouped = rows.filter((row) => row.kind === "Group");
              const groups: GroupSummary = {
                entries: grouped.length,
                souls: grouped.reduce((sum, row) => sum + row.souls, 0),
                people: rows
                  .filter((row) => row.kind === "Person")
                  .reduce((sum, row) => sum + row.souls, 0),
                largest: grouped.reduce((max, row) => Math.max(max, row.souls), 0),
              };
              download(
