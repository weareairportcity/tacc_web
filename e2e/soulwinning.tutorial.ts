...
test("entry tutorial video", async ({ browser }) => {
  ...
  await title(page, "How to log a soul", "1909 · members · one minute");
  await hold(page, 4200);
  await page.evaluate(() => window.__swTutorial?.hideTitle());
  await hold(page, 900);

  await say(page, 1, "First time? Start new. Already have a code from another phone? Use that.");
  await highlight(page, page.getByRole("button", { name: "Start new" }), { circle: true });
  await hold(page, 2200);
  await highlight(page, page.getByRole("button", { name: "I have a code" }), { circle: true });
  await hold(page, 2800);
  await page.getByRole("button", { name: "Start new" }).click();
  await expect(page.getByRole("heading", { name: "Before you start" })).toBeVisible();

  // 2. Member details
  await say(page, 2, "Who is entering? That's you. A member.");
  await highlight(page, page.getByRole("textbox", { name: "Name" }), { circle: true });
  await hold(page, 2600);
  await typeInto(page, page.getByRole("textbox", { name: "Name" }), "Kofi Mensah");
  await hold(page, 700);
  await highlight(page, page.getByLabel("Fellowship"), { circle: true });
  await hold(page, 1800);
  await page.getByLabel("Fellowship").selectOption("Qadash");
  await hold(page, 900);
  await highlight(page, page.getByRole("textbox", { name: "Number" }), { circle: true });
  await hold(page, 450);
  await typeInto(page, page.getByRole("textbox", { name: "Number" }), "0244123456");
  await hold(page, 800);
  await highlight(page, page.getByRole("button", { name: "Start logging" }), { circle: true });
  await hold(page, 2200);
  ...
  await say(page, 3, "Turn on location. Every soul is pinned to the spot you won them.");
  ...
  await say(page, 4, "Need your code for another phone? Open Switch — it lives there, off this screen.");
  await highlight(page, page.getByRole("button", { name: /Switch/ }), { circle: true });
  ...
  await highlight(page, page.locator("[data-login-code]"), { circle: true });
  await hold(page, 4200);
  await page.getByRole("button", { name: "Close" }).click();
  await expect(page.getByPlaceholder("Name")).toBeVisible();

  // 5. Soul name + phone
  await say(page, 5, "Now the person you just led — name and phone.");
  const soulName = page.getByPlaceholder("Name");
  await highlight(page, soulName, { circle: true });
  await hold(page, 2200);
  await typeInto(page, soulName, "Kwame Mensah");
  await hold(page, 700);
  await highlight(page, page.getByPlaceholder("Phone"), { circle: true });
  await hold(page, 1400);
  await typeInto(page, page.getByPlaceholder("Phone"), "0244001122");
  await hold(page, 900);

  // 4. Photo
  await say(page, 6, "A photo is optional. Take one now, or upload from the phone.");
  await highlight(page, page.getByRole("button", { name: "Take photo" }), { circle: true });
  await hold(page, 2200);
  await highlight(page, page.getByRole("button", { name: "Upload photo" }), { circle: true });
  await hold(page, 1800);
  await page.getByLabel("Upload photo for soul 1").setInputFiles(PHOTO);
  await expect(page.getByText("Photo added")).toBeVisible();
  await highlight(page, page.getByText("Photo added"), { circle: true });
  await hold(page, 2200);

  // 5. Toggles
  await say(page, 6, "Tongues? Coming to church? Flip whatever is true.");
  const tongues = page.getByRole("switch", { name: "Spoke in tongues" });
  const church = page.getByRole("switch", { name: "Coming to church" });
  ...
  await say(page, 7, "Save. It stays on this phone even if you're offline.");
  const save = page.getByRole("button", { name: "Save soul" });
  ...
  await say(page, 8, "My souls is everyone you've logged on this phone.");
  ...
  await highlight(page, page.getByText("Kwame Mensah").first(), { circle: true });
  await hold(page, 4200);
  await page.evaluate(() => window.__swTutorial?.clearHighlight());
  await say(page, 8, "That's it. One soul, credited to you.");
  await hold(page, 3200);
  ...
  const webm = path.join(OUT_DIR, "entry-tutorial.webm");
  const mp4 = path.join(OUT_DIR, "entry-tutorial.mp4");
  ...
  await page.evaluate(() => {
    ...
    root.innerHTML = `
      ...
      <div class="sw-t-title">
        <h1>How to log a soul</h1>
        <p>1909 · members · one minute</p>
      </div>
    `;
...
async function highlight(page: Page, locator: Locator, opts?: { circle?: boolean; pad?: number }) {
